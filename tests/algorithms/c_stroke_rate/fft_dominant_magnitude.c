...
static int compute_spm(void)
{
    ...
    int64_t neighbor_power[2] = {0, 0};  /* [prev_bin, next_bin] for parabolic interp */

    int32_t sum = 0;
    for (int i = 0; i < FFT_SIZE; i++) sum += filtered[i];
    int16_t mean = (int16_t)(sum / FFT_SIZE);

    /* Goertzel algorithm for each bin in cadence range */
    ...
    /* Compute neighbor bin powers for parabolic interpolation */
    for (int d = -1; d <= 1; d += 2) {
        int nb = best_bin + d;
    ...
    /* Parabolic interpolation: delta = 0.5 * (alpha - gamma) / (alpha - 2*beta + gamma) */
...
int main(void)
{
    ...
    while (fgets(line, sizeof(line), stdin) != NULL) {
        ...
        for (int i = 0; i < SAMPLE_STORE_CAPACITY; i++) {
            ...
            filtered[i] = filters_hpf_gravity_process(&hpf, mag);
        }
